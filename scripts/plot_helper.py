...
def plot_vector(vectors, tails=None):
    ...
    axis.quiver(tails[:, 0], tails[:, 1], vectors[:, 0], vectors[:, 1], color=darkblue,
                angles='xy', scale_units='xy', scale=1, width=0.022)
...
def plot_transformation_helper(axis, matrix, *vectors, unit_vector=True, unit_circle=False, title=None):
    ...
    *vectors : class numpy.ndarray.
    ...
    """
    assert matrix.shape == (
        2, 2), "the input matrix must have a shape of (2,2)"
    grid_range = 20
    ...
    for i in range(x.size):
        axis.plot(X[i, :], Y[i, :], **grid_params)
        axis.plot(X[:, i], Y[:, i], **grid_params)"""
    axis.grid(True, **grid_params)

    # draw (transformed) unit vectors
    if unit_vector:
        axis.quiver(origin, origin, [I[0]], [I[1]],
                    color=green, **quiver_params)
    ...
    if vectors:
        for vector in vectors:
            ...
            axis.quiver(origin, origin, [vector_[0]], [
                        vector_[1]], color=color, **quiver_params)
...
def plot_linear_transformation(matrix, *vectors, unit_vector=True, unit_circle=False):
    ...
    *vectors : class numpy.ndarray.
    ...
    plot_transformation_helper(axis1, numpy.identity(
        2), *vectors, unit_vector=unit_vector, unit_circle=unit_circle, title='Before transformation')
    plot_transformation_helper(axis2, matrix, *vectors, unit_vector=unit_vector,
                               unit_circle=unit_circle, title='After transformation')
...
def plot_linear_transformations(*matrices, unit_vector=True, unit_circle=False):
    ...
    *matrices : class numpy.ndarray.
    ...
    for i in range(nplots):  # fig_idx
        ...
        plot_transformation_helper(axes[i//nx, i % nx], matrix_trans,
                                   unit_vector=unit_vector, unit_circle=unit_circle, title=title)
...
def plot_3d_transformation_helper(axis, matrix, grid=True, unit_sphere=False, title=None):
    ...
    """
    assert matrix.shape == (
        3, 3), "the input matrix must have a shape of (3,3)"
    ...
    if grid:
        ...
        for i in range(x.size):
            for j in range(x.size):
                axis.plot(X_new[:, i, j], Y_new[:, i, j],
                          Z_new[:, i, j], color=xcolor, linewidth=linewidth)
                axis.plot(X_new[i, :, j], Y_new[i, :, j],
                          Z_new[i, :, j], color=ycolor, linewidth=linewidth)
                axis.plot(X_new[i, j, :], Y_new[i, j, :],
                          Z_new[i, j, :], color=zcolor, linewidth=linewidth)
    ...
    if unit_sphere:
        ...
        Y_new = matrix[1, 0]*X + matrix[1, 1]*Y + matrix[1, 2]*Z
        Z_new = matrix[2, 0]*X + matrix[2, 1]*Y + matrix[2, 2]*Z
        axis.plot_surface(X_new, Y_new, Z_new, rstride=4,
                          cstride=4, linewidth=0, cmap='ocean', alpha=0.6)
...
def plot_3d_linear_transformation(matrix, grid=True, unit_sphere=False):
    ...
    plot_3d_transformation_helper(axis1, numpy.identity(
        3), grid=grid, unit_sphere=unit_sphere, title='before transformation')
...
def plot_basis_helper(axis, I, J, *vectors, title=None, I_label='i', J_label='j'):
    ...
    *vectors : class numpy.ndarray.
    ...
    axis.quiver(origin, origin, [J[0]], [J[1]],
                color=lightblue, **quiver_params)
    ...
    if vectors:
        for vector in vectors:
            ...
            axis.quiver(origin, origin, [vector[0]], [
                        vector[1]], color=red, **quiver_params)
    ...
    axis.text((I[0]-J[0])/2*1.1, (I[1]-J[1])/2*1.1,
              r'${}$'.format(I_label), color=gold, **text_params)
    axis.text((J[0]-I[0])/2*1.1, (J[1]-I[1])/2*1.1,
              r'${}$'.format(J_label), color=lightblue, **text_params)
...
def plot_change_basis(I, J, *vectors):
    ...
    *vectors : class numpy.ndarray.
    ...
    plot_basis_helper(axis1, numpy.array([1, 0]), numpy.array(
        [0, 1]), *vectors, title='standard basis')
    plot_basis_helper(axis2, I, J, *vectors_,
                      title='new basis', I_label='a', J_label='b')
...
def plot_eigen(matrix):
    ...
    plot_basis_helper(axes[0, 0], numpy.array([1, 0]), numpy.array(
        [0, 1]), title=r'coords in standard basis $\mathbf{x}$')
    plot_basis_helper(axes[0, 1], C[:, 0], C[:, 1],
                      title=r'change to new basis $C^{-1}\mathbf{x}$', I_label='a', J_label='b')
    plot_basis_helper(axes[1, 0], C[:, 0], C[:, 1],
                      title=r'scale along new basis $DC^{-1}\mathbf{x}$', I_label='a', J_label='b')
    plot_basis_helper(axes[1, 1], numpy.array([1, 0]), numpy.array(
        [0, 1]), title=r'change back to standard basis $CDC^{-1}\mathbf{x}$')
